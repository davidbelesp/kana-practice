    ...
    <>
      ...
      <nav className={`navbar ${isFocus ? "navbar-focus" : ""}`} aria-label={title}>
        ...
        <div className="navbar-right">
          ...
          {!isFocus && <Link to="/settings" className="btn-icon" aria-label={t("common.settings")}><Settings size={17} strokeWidth={1.8} /></Link>}
        </div>
      </nav>
      ...
      {!isFocus && (
        <>
          ...
          <aside className={`navbar-drawer ${isOpen ? "open" : ""}`} aria-label={t("nav.navigation")} aria-hidden={!isOpen}>
            ...
            <nav className="drawer-nav">{links.map(({ to, label, Icon, active }) => <Link key={to} to={to} onMouseEnter={() => prefetchRoute(to)} onFocus={() => prefetchRoute(to)} className={`drawer-link ${active ? "active" : ""}`} onClick={close}><Icon size={17} /><span>{label}</span></Link>)}</nav>
          </aside>
