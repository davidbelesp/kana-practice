    ...
    <>
      ...
      <nav className={`navbar ${isFocus ? "navbar-focus" : ""}`} aria-label={title}>
        ...
        <div className="navbar-right">
          ...
          {!isFocus && <>
            {!user ? <div className="navbar-account-guest"><Link to="/create-account" className="navbar-account-link" onMouseEnter={() => prefetchRoute("/create-account")} onFocus={() => prefetchRoute("/create-account")}>{t("nav.signIn")}</Link><Link to="/login" className="navbar-account-link navbar-account-link-muted" onMouseEnter={() => prefetchRoute("/login")} onFocus={() => prefetchRoute("/login")}>{t("nav.login")}</Link></div> : <Link to="/settings?tab=account" className="account-avatar" aria-label={t("nav.accountAvatar", { username: accountLabel })} title={accountLabel}>{accountInitial}</Link>}
