...
import { getKanaMasteryState, getKanaProgressScore, isKanaMastered } from "../utils/kanaMastery";
...
export const Stats = () => {
  ...
  const grammarItems = itemsFor(items, "grammar");
  const grammarTotal = getGrammarCatalogExerciseCount();
  const grammarMastered = grammarItems.filter((item) => item.masteryScore >= 85).length;
  ...
  const renderHeatmap = (label: string, characters: typeof allKanaData) => (
    <div className="progress-heatmap-group">
      <div className="progress-subheading"><span>{label}</span><span className="progress-count">{characters.filter((item) => getKanaProgressScore(kanaMap.get(item.char)?.streak ?? 0) > 0).length}/{characters.length}</span></div>
      <div className="kana-heatmap" aria-label={label}>{characters.map((item) => { const score = getKanaProgressScore(kanaMap.get(item.char)?.streak ?? 0); const masteryState = getKanaMasteryState(score, settings.masteryThreshold); return <Link key={item.char} to={`/practice?char=${encodeURIComponent(item.char)}`} className={`heatmap-tile ${masteryState === "mastered" ? "is-mastered" : masteryState}`} title={`${item.char} · ${score}%`}><span>{item.char}</span><small>{item.romaji}</small></Link>; })}</div>
    </div>
  );
  ...
  return (
    <AppShell title={t("stats.title")}>
      <div className="progress-container">
        <header className="progress-hero"><div><span className="eyebrow">{t("stats.commandCenter")}</span><h1>{t("stats.title")}</h1><p>{t("stats.subtitle")}</p></div><Link className="btn-primary" to={recommendations[0]?.actionPath ?? "/practice"}>{t("stats.startPractice")}</Link></header>
        ...
        <div className="progress-layout">
          ...
          <section className="progress-panel glass-panel progress-wide-panel"><div className="panel-heading"><div><span className="eyebrow">{t("stats.numberLab")}</span><h2>{t("stats.numbersProgress")}</h2></div><Link className="btn-text" to="/numbers">{t("stats.openDomain")} →</Link></div>{numberItems.length ? <div className="number-progress-list">{NUMBER_GROUPS.map((group) => { const item = numberItems.find((entry) => entry.itemId === `range-${group}`); if (!item) return null; const total = item.correct + item.incorrect; return <div className="number-progress-row" key={group}><span>{t("stats.digitGroup", { group })}</span><div className="progress-track"><i style={{ width: `${item.masteryScore}%` }} /></div><strong>{item.masteryScore}%</strong><small>{total}</small></div>; })}</div> : <div className="panel-placeholder">{t("stats.numbersNoData")}</div>}</section>
          <section className="progress-panel glass-panel"><div className="panel-heading"><div><span className="eyebrow">{t("stats.grammarPath")}</span><h2>{t("stats.grammarProgress")}</h2></div><Link className="btn-text" to="/grammar">{t("stats.openDomain")} →</Link></div><div className="domain-stat-large"><strong>{grammarMastered}/{grammarTotal}</strong><span>{t("stats.grammarExercisesMastered")}</span></div><p className="panel-muted">{grammarItems.length ? t("stats.grammarExercisesPracticed", { count: grammarItems.length }) : t("stats.grammarNoData")}</p></section>
          <section className="progress-panel glass-panel progress-wide-panel"><div className="panel-heading"><div><span className="eyebrow">{t("stats.recentLog")}</span><h2>{t("stats.recentSessions")}</h2></div></div>{sessions.length ? <div className="session-list">{sessions.sort((a, b) => b.completedAt - a.completedAt).slice(0, 8).map((session) => <div className="session-row" key={session.id}><span>{t(domainLabelKey(session.domain))}</span><small>{session.mode}</small><strong>{session.correct}/{session.total}</strong><small>{session.accuracy}%</small><time>{dayLabel(session.completedAt)}</time></div>)}</div> : <div className="panel-placeholder">{t("stats.noHistory")}</div>}</section>
        </div>
      </div>
    </AppShell>
  );
